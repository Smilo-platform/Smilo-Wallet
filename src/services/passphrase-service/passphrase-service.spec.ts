...
describe("PassphraseService", () => {
    ...
    beforeEach(() => {
        ...
        service = new PassphraseService(httpClient);
    });

    it("should generate correct passphrases", (done) => {
        let promises: Promise<void>[] = [];

        for(let testSet of correctPassphrases.english) {
            promises.push(
                service.generate(testSet.length / 2 * 8).then(
                    (result) => {
                        expect(result.entropy).toEqual(testSet[0], "Entropy should match");
                        expect(result.passphrase).toEqual(testSet[1].split(" "), "Passphrase should match");
                        expect(result.seed).toEqual(testSet[2], "Seed should match");
                        expect(result.key).toEqual(testSet[3], "Key should match");
                    }
                )
            );
        }
        ...
        Promise.all(promises).then(
            ...
            (error) => {
                console.error(error);
                expect(true).toBe(false, "No promise reject should be raised.");
