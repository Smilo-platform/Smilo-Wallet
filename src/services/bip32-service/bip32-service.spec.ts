...
describe("BIP32Service", () => {
    ...
    it("should call bitcoinjs correctly", () => {
        ...
        expect(bitcoinjs.getPrivateKeyFromSeed).toHaveBeenCalledWith("SEED", 10, undefined);
    });
    ...
    it("should return correct private keys", () => {
        for(let testVector of testVectors) {
            for(let variation of testVector.variations) {
                ...
                expect(service.getPrivateKey(testVector.seed, variation.index, testVector.coinType)).toBe(variation.privateKey);
            }
        }
    });
});
