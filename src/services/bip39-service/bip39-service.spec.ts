...
describe("BIP39Service", () => {
    ...
    beforeEach(() => {
        ...
        service = new BIP39Service(httpClient, translateService);
    });

    beforeEach(() => {
        // Only way to retrieve global library
        ...
        for(let i = 0; i < phraseTestVectors.english.length; i++) {
            promise = promise.then(
                () => {
                    ...
                    let strength = (testVector[0].length / 2) * 8;

                    return service.generate(strength).then(
                        (phrase) => {
        ...
        promise.then(
            () => {
                for(let i = 0; i < generatedPhrases.length; i++) {
                    let phrase = generatedPhrases[i];

                    let testVector = phraseTestVectors.english[i];

                    expect(phrase).toBe(testVector[1]);
                }
    ...
    it("should generate correct seeds", () => {
        for(let i = 0; i < phraseTestVectors.english.length; i++) {
            let testVector = phraseTestVectors.english[i];
